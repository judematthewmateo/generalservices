...
export default {
    ...
    onPurchasemainEntry() {
      if(this.tables.orderlists.items.length > 0) {
        ...
        if (this.entryMode == "Add") {
          ...
          this.updateEntity("purchasemain", "purchase_order_id", false, this.row)
        }
      }
      else {
        this.$notify({
    ...
    onPurchasemainDelete() {
      this.deleteEntity("purchasemain", this.purchase_order_id, true, "purchasemains", "purchase_order_id")
    },
    async setDelete(data) {
      if((await this.checkIfUsed("purchasemain", data.item.purchase_order_id)) == true){
        this.$notify({
          type: "error",
          group: "notification",
          title: "Error!",
    ...
    setUpdate(data) {
      this.row = data.item
      this.$http.get('api/purchasemain/'+ data.item.purchase_order_id, {
          headers: {
  ...
  created() {
    ...
    this.$http.get('api/purchasemains', {
        headers: {
            Authorization: 'Bearer ' + localStorage.getItem('token')
        }
    }).then(response => {
        this.tables.purchasemains.items = response.data.purchasemains
        this.paginations.purchasemains.totalRows = response.data.purchasemains.length
  ...
  computed: {
    getTotalItems() {
      ...
      this.tables.orderlists.items.forEach(item => {
            this.forms.purchasemain.fields.total_amount += Number(item.product_cost) * Number(item.product_quantity)
